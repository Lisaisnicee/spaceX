...
const MemberDetails = () => {
        ...
        <div className="slide">  
          <div className="card">
            ...
            <div className="card__content"> 
              ...
              <p className="card__content-status">{member.status ? 'Active' : 'Inactive'}</p>
              <Link className="card__content-link" to={`${member.wikipedia}`}>En savoir plus sur {member.name}</Link>
            </div>
          </div>
        </div>
    );
  ...
  return (
    <Fetcher url="https://api.spacexdata.com/v4/crew" render={renderMemberDetails} />
  );
};
